...
resource "aws_subnet" "private_subnet" {
  for_each          = { for index, az_name in slice(data.aws_availability_zones.this.names, 1, 3) : index => az_name }
  vpc_id            = aws_vpc.this.id
  cidr_block        = cidrsubnet(var.cidr_for_vpc, length(data.aws_availability_zones.this.names) > 3 ? 3 : 2, each.key)
...
resource "aws_subnet" "public_subnet" {
  for_each                = { for index, az_name in slice(data.aws_availability_zones.this.names, 1, 3) : index => az_name }
  vpc_id                  = aws_vpc.this.id
  cidr_block              = cidrsubnet(var.cidr_for_vpc, length(data.aws_availability_zones.this.names) > 3 ? 3 : 2, each.key + length(data.aws_availability_zones.this.names))
  availability_zone       = each.value
...
resource "aws_eip" "nat" {
  # for_each      = [ for each_subnet in aws_subnet.public_subnet : each_subnet.id ]
  # for_each      = { for index, az_name in slice(data.aws_availability_zones.this.names, 1, 3) : index => az_name } to be corrected!
  domain = "vpc"
